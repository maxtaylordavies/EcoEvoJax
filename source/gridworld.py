...
def get_ob(state: jnp.ndarray, pos_x: jnp.int32, pos_y: jnp.int32) -> jnp.ndarray:
    obs = jax.lax.dynamic_slice(
        jnp.pad(state, ((AGENT_VIEW, AGENT_VIEW), (AGENT_VIEW, AGENT_VIEW), (0, 0))),
...
def get_init_state_fn(
    key: jnp.ndarray, SX, SY, posx, posy, pos_food_x, pos_food_y, niches_scale=200
) -> jnp.ndarray:
    ...
    new_array = jnp.clip(
        np.asarray(
            [
                (math.pow(niches_scale, el) - 1) / (niches_scale - 1)
                for el in np.arange(0, SX) / SX
            ]
        ),
        0,
        1,
...
class Gridworld(VectorizedTask):
    ...
    def __init__(
        ...
        self.model = MetaRnnPolicy_bcppr(
            input_dim=((AGENT_VIEW * 2 + 1), (AGENT_VIEW * 2 + 1), 3),
            hidden_dim=4,
            output_dim=5,
            encoder_layers=[],
            hidden_layers=[8],
        ...
        def reset_fn(key, agent_locs=None, resource_locs=None):
            # initialise agent locations
            if agent_locs is not None:
                assert agent_locs.shape[0] == self.nb_agents
                posx = agent_locs[:, 0]
                posy = agent_locs[:, 1]
            elif self.place_agent:
                next_key, key = random.split(key)
                posx = random.randint(
                    next_key, (nb_agents,), int(2 / 5 * SX), int(3 / 5 * SX)
                )
                next_key, key = random.split(key)
                posy = random.randint(
                    next_key, (nb_agents,), int(2 / 5 * SX), int(3 / 5 * SX)
            ...
            # initialise food locations
            if resource_locs is not None:
                assert resource_locs.shape[0] == init_food
                pos_food_x = resource_locs[:, 0]
                pos_food_y = resource_locs[:, 1]
            elif self.place_resources:
                ...
                pos_food_x = jnp.concatenate(
                    (
                        ...
                        random.randint(
                            next_key, (int(init_food / 4),), N_wall, int(1 / 2 * SX) - N
                        ),
                        random.randint(
                            next_key,
                            (int(init_food / 4),),
                ...
                pos_food_y = jnp.concatenate(
                    (
                        random.randint(
                            next_key, (int(init_food / 4),), 1 + N_wall, SY - 1 - N_wall
                        ),
                        random.randint(
                            next_key, (int(init_food / 4),), 1 + N_wall, SY - 1 - N_wall
                        ),
                        ...
                        random.randint(
                            next_key, (int(init_food / 4),), N_wall, int(1 / 2 * SY) - N
                        ),
                    )
                )
            else:
            ...
            next_key, key = random.split(key)
            grid = get_init_state_fn(
                key, SX, SY, posx, posy, pos_food_x, pos_food_y, niches_scale
            )
            ...
            policy_states = self.model.reset_b(
                jnp.zeros(
                    self.nb_agents,
                )
            )
            ...
            # alive = (
            #     jnp.ones((self.nb_agents,), dtype=jnp.uint16)
            #     .at[0 : 2 * self.nb_agents // 3]
            #     .set(0)
            # )
            energy = self.max_ener * jnp.ones((self.nb_agents,))
            alive = jnp.ones((self.nb_agents,), dtype=jnp.uint16)
            ...
            return State(
                ...
                last_actions=jnp.zeros((self.nb_agents, 5)),
                rewards=jnp.zeros((self.nb_agents, 1)),
                agents=agents,
        ...
        def reproduce(
            ...
            empty_spots = jax.random.choice(
                next_key,
                jnp.arange(time_good_level.shape[0]),
                p=dead,
                replace=False,
                shape=(5,),
            )
            ...
            reproducer_spots = jax.random.choice(
                next_key,
                jnp.arange(time_good_level.shape[0]),
                p=reproducer / (reproducer.sum() + 1e-10),
                replace=False,
                shape=(5,),
            )
            ...
            if self.reproduction_on:
                params = params.at[empty_spots].set(
                    params[reproducer_spots]
                    + 0.02 * jax.random.normal(next_key, (5, params.shape[1]))
                )
            ...
            energy = energy.at[empty_spots].set(
                self.max_ener * reproducer[reproducer_spots]
            )
            energy = energy.at[0:5].set(0.0)
            ...
            policy_states = metaRNNPolicyState_bcppr(
                lstm_h=policy_states.lstm_h.at[empty_spots].set(
                    jnp.zeros(policy_states.lstm_h.shape[1])
                ),
                lstm_c=policy_states.lstm_c.at[empty_spots].set(
                    jnp.zeros(policy_states.lstm_c.shape[1])
                ),
        ...
        def step_fn(state, override_actions: Dict = {}):
            key = state.key
            next_key, key = random.split(key)

            # model selection of action
            actions_logit, policy_states = self.model.get_actions(
                state, state.agents.params, state.agents.policy_states
            )
            print(f"actions_logit ({actions_logit.shape}): {actions_logit}")
            actions = jax.nn.one_hot(
                jax.random.categorical(next_key, actions_logit * 50, axis=-1), 5
            )
            print(f"actions before ({actions.shape}): {actions}")
            # actions=jax.nn.one_hot(jnp.argmax(actions_logit,axis=-1),5)

            for agent_idx, action in override_actions.items():
                actions = actions.at[agent_idx].set(action)

            print(f"actions after ({actions.shape}): {actions}")
            ...
            num_neighbs = jax.scipy.signal.convolve2d(
                grid[:, :, 1], jnp.array([[0, 1, 0], [1, 0, 1], [0, 1, 0]]), mode="same"
            )
            scale = grid[:, :, 3]
            ...
            if scale_constant:
                ...
                num_neighbs = jnp.where(
                    num_neighbs == 2, 0.01 / scale_constant, num_neighbs
                )
                num_neighbs = jnp.where(
                    num_neighbs == 3, 0.05 / scale_constant, num_neighbs
                )
            ...
            time_good_level = jnp.where(
                energy > 0, (state.agents.time_good_level + 1) * alive, 0
            )
            ...
            reproducer = jnp.where(
                state.agents.time_good_level > self.time_reproduce, 1, 0
            )
            next_key, key = random.split(key)
            (
                params,
                ...
                reproducer.sum() > 0,
                reproduce,
                lambda y, z, a, b, c, d, e, f, g, h, i: (y, z, a, b, c, e, f, g, h, i),
                *(
                    state.agents.params,
                    posx,
                    posy,
                    ...
                    alive,
                    nb_food,
                    state.agents.nb_offspring,
                ),
            )

            time_under_level = jnp.where(
                energy < 0, state.agents.time_under_level + 1, 0
            )
            alive = jnp.where(
                jnp.logical_or(
                    time_alive > self.max_age, time_under_level > self.time_death
                ),
                0,
                alive,
            )
            ...
            cur_state = State(
                ...
                agents=AgentStates(
                    ...
                    posy=posy,
                    energy=energy,
                    time_good_level=time_good_level,
                    params=params,
                    policy_states=policy_states,
                    time_alive=time_alive,
                    time_under_level=time_under_level,
                    alive=alive,
                    nb_food=nb_food,
                    nb_offspring=nb_offspring,
                ),
                steps=steps,
                key=key,
            )
    ...
    def reset(
        self,
        key: jnp.ndarray,
        agent_locs: Optional[jnp.ndarray] = None,
        resource_locs: Optional[jnp.ndarray] = None,
    ) -> State:
        return self._reset_fn(key, agent_locs, resource_locs)

    def step(
        self, state: State, override_actions: Dict = {}
    ) -> Tuple[State, jnp.ndarray, jnp.ndarray]:
        return self._step_fn(state, override_actions=override_actions)
